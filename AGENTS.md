...
- `engine.py`: `tick_loop(pub)` (sole writer, 1 tps, spawn at gold>=`SPAWN_AT`=100 then reset, ~100s natural spawn) + `cmd_listener(pub)` (sole reader). Separate Redis conns for pub vs sub; `asyncio.gather` both in `main()`. Pass `pub` as arg, never global-None.
...
- `gateway.py`: `redis_pump()` (single task, subscribes `game.state` + `game.events` + `narrator.broadcast`, envelopes `{"type","data"}`, drops zombies on send error) + `/ws` endpoint + `GET /` serving `index.html`. `index.html`: pixel-art battlefield (offscreen bg, smoothing off, 3 lanes, castles, marching soldiers from spawn events, clash marks, siege HP cosmetic-only, campaign clock, feed). Viewer is READ-ONLY — never publishes back to Redis.
